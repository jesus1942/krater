...
s = p.read_text()
imp = "use Crater\\Http\\Controllers\\V1\\Family\\FamilyMembersController;\n"
anchor = "use Crater\\Http\\Controllers\\V1\\Expense\\UploadReceiptController;\n"
if imp not in s:
    s = s.replace(anchor, anchor + imp)
route_block = """        // Families / responsible adults (canonical family_members)\n        //----------------------------------\n\n        Route::get('/family-members', [FamilyMembersController::class, 'index']);\n        Route::post('/family-members', [FamilyMembersController::class, 'store']);\n        Route::put('/family-members/{familyMember}', [FamilyMembersController::class, 'update']);\n\n\n"""
anchor2 = "        // Students / school records\n        //----------------------------------\n"
if "Route::get('/family-members'" not in s:
    s = s.replace(anchor2, route_block + anchor2)
p.write_text(s)

# Canonical families screen, replacing legacy customer listing.
vue = r'''<template>
  <base-page>
...
'''
(root / 'resources/assets/js/views/customers/Index.vue').write_text(vue)

# Static regression checks for the canonical source and non-destructive UI.
test = r'''<?php
...
'''
(root / 'tests/Isolation/FamilyMembersCanonicalTest.php').write_text(test)

# remove this transformer after applying; it must not ship.
Path('scripts/apply_family_members_ui.py').unlink()

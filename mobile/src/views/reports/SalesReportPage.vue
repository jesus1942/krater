    ...
    <ion-content color="light">
      ...
      <template v-else-if="report">
        <ion-card>
          <ion-card-content class="ion-no-padding">
            <ion-list lines="full">
              <ion-item><ion-label>Total facturado</ion-label><ion-note slot="end" color="success">{{ formatMoney(report.total_amount) }}</ion-note></ion-item>
              <ion-item><ion-label>Total cobrado</ion-label><ion-note slot="end" color="primary">{{ formatMoney(report.paid_amount) }}</ion-note></ion-item>
              <ion-item lines="none"><ion-label>Total pendiente</ion-label><ion-note slot="end" color="danger">{{ formatMoney(report.due_amount) }}</ion-note></ion-item>
            </ion-list>
          </ion-card-content>
        </ion-card>

        <ion-card v-if="report.invoices?.length">
          <ion-card-header><ion-card-title>Facturas</ion-card-title></ion-card-header>
          <ion-card-content class="ion-no-padding">
            <ion-list lines="full">
              <ion-item v-for="inv in report.invoices" :key="inv.id" button :router-link="`/invoices/${inv.id}`">
                <ion-label><h3>{{ inv.invoice_number }}</h3><p>{{ inv.customer?.name }}</p></ion-label>
                <ion-note slot="end">{{ formatMoney(inv.total) }}</ion-note>
              </ion-item>
            </ion-list>
          </ion-card-content>
        </ion-card>
...
async function load() {
  loading.value = true
  try {
    const { data } = await http.get('/reports/sales', { params: { from_date: from.value, to_date: to.value } })
    report.value = data
